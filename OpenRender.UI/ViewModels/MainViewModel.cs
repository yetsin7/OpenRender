...
public enum LumionWorkspaceTool
{
    Import,
    Build,
    Materials,
...
public partial class MainViewModel : ObservableObject
{
    ...
    private async Task LoadFileAsync(string filePath)
    {
        ...
        try
        {
            ...
            var manager = new OpenRender.Core.Import.ImportManager();
    ...
    partial void OnCurrentSourceFilePathChanged(string? value) => OnPropertyChanged(nameof(HasLoadedSourceFile));
    partial void OnActiveSidePanelChanged(LumionSidePanel value) => NotifyLumionPanelProperties();
    ...
    private async Task RunNavigationSmokeTestAsync(string? capturePath)
    {
        ...
        {
            // await ViewportCaptureService.CaptureAsync(WithSuffix(capturePath, "_front"), _renderSettings.Width, _renderSettings.Height, _renderSettings.Format, _renderSettings.JpegQuality);
        }
        ...
        if (!string.IsNullOrWhiteSpace(capturePath))
        {
            // await ViewportCaptureService.CaptureAsync(WithSuffix(capturePath, "_top"), _renderSettings.Width, _renderSettings.Height, _renderSettings.Format, _renderSettings.JpegQuality);
        }
        ...
        if (!string.IsNullOrWhiteSpace(capturePath))
        {
            // await ViewportCaptureService.CaptureAsync(capturePath, _renderSettings.Width, _renderSettings.Height, _renderSettings.Format, _renderSettings.JpegQuality, cleanViewport: true);
