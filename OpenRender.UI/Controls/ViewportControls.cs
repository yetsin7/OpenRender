...
public class VulkanViewportControl : NativeControlHost
{
    ...
    private Scene3D? _boundScene;
    private Scene3D? _pendingSceneUpload;
                ...
                try
                {
                    ...
                    MainViewModel.ReportViewportReady(
                        _renderer?.FrameSubmitEnabled != true
                            ...
                            : "Vulkan inicializo estable con malla real, depth buffer y present seguro. GBuffer/SSAO queda detras de OPENRENDER_ENABLE_VULKAN_ADVANCED_PIPELINE=1.");
                }
                catch (Exception ex)
                {
    ...
    private void RenderLoop()
    {
        try
        {
            while (_isRunning)
            {
                UploadPendingSceneIfNeeded();

                _time.Update();
                _cameraController.Update(ActiveScene.Camera, _input, _time);

                float aspect = (float)(Bounds.Width / Bounds.Height);
                if (aspect < 0.1f) aspect = 1.0f;

                Matrix4x4 view = Matrix4x4.CreateLookAt(ActiveScene.Camera.Position, ActiveScene.Camera.Target, Vector3.UnitY);
                Matrix4x4 proj = Matrix4x4.CreatePerspectiveFieldOfView(MathHelper.ToRadians(ActiveScene.Camera.FieldOfView), aspect, ActiveScene.Camera.NearPlane, ActiveScene.Camera.FarPlane);
                proj.M22 *= -1;

                Matrix4x4 viewProjection = view * proj;
                _input.ClearDelta();
                _renderer?.Render(viewProjection, ActiveScene.Camera.Position, _time.TotalTime, proj);
                Thread.Sleep(1);
            }
        }
        catch (Exception ex)
    ...
    {
        float aspect = (float)(Bounds.Width / Bounds.Height);
        float fovRad = MathHelper.ToRadians(ActiveScene.Camera.FieldOfView);

        float nx = (float)(2.0 * mousePos.X / Bounds.Width - 1.0);
        float ny = (float)(1.0 - 2.0 * mousePos.Y / Bounds.Height);

        Matrix4x4 view = Matrix4x4.CreateLookAt(ActiveScene.Camera.Position, ActiveScene.Camera.Target, Vector3.UnitY);
        Matrix4x4 proj = Matrix4x4.CreatePerspectiveFieldOfView(fovRad, aspect, ActiveScene.Camera.NearPlane, ActiveScene.Camera.FarPlane);
        ...
        var hitNode = _selectionSystem.Pick(ray, ActiveScene);
    ...
    private Scene3D ActiveScene => _boundScene ?? Scene;

    private void OnViewModelPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(MainViewModel.Scene) && _viewModel != null)
            BindScene(_viewModel.Scene);
    }

    private void BindScene(Scene3D? scene)
    {
        _boundScene = scene;
        _pendingSceneUpload = scene;
    }

    private void UploadPendingSceneIfNeeded()
    {
        if (_pendingSceneUpload == null || _renderer == null)
            return;

        _renderer.LoadScene(_pendingSceneUpload);
        _pendingSceneUpload = null;
    }
